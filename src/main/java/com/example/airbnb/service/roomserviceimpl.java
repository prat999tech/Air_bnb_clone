...
public class roomserviceimpl implements RoomService {
    ...
    public RoomDto createNewRoom(long hotelId, RoomDto roomDto) {
        ...
        if(!user.equals(hotel.getOwner())) {
            log.warn("user with id: {} is not the owner of the hotel with id: {}", user.getId(), hotelId);
    ...
    public void deleteRoomById(Long roomId) {
        ...
        if(!user.equals(room.getHotel().getOwner())) {
            log.warn("user with id: {} is not the owner of the hoteland also room with id: {}", user.getId(), room.getHotel().getId(),room.getId());
            throw new Unauthorizedexception("You are not the owner of this hotel and room ");
        }
    ...
    public RoomDto updateRoomById(Long hotelId, Long roomId, RoomDto roomDto) {
        ...
        room.setId(roomId); // we have done this because suppose we donot have id parameter in roomdto so to prevent the nullpointer exception so we explicitly set the roomid
        room = roomRepository.save(room);
        return modelMapper.map(room, RoomDto.class);
    }
